...
describe('authService.loginUser', () => {
    ...
    const mockUser = { id: 1, email: 'test@example.com', password: 'hashedPassword', isActive: true };
  ...
  it('should return an error if the email does not exist', async () => {
    ...
    const result = await authService.loginUser('notfound@example.com', 'password');
  ...
  it('should return an error if the user exists but the account is not active', async () => {
    const mockUser = { id: 1, email: 'test@example.com', password: 'hashedPassword', isActive: false };
    (User.findOne as jest.Mock).mockResolvedValue(mockUser);

    const result = await authService.loginUser('test@example.com', 'password');

    expect(result).toEqual({ error: 'Compte non activé. Merci de vérifier votre email.' });
  });
