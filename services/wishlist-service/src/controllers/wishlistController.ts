...
export const updateWishlist = asyncHandler(
  async (req: AuthenticatedRequest, res, next) => {
    ...
    if (wishlist.userId !== req.user.userId) {
      return next(
...
export const updateWish = asyncHandler(
  async (req: AuthenticatedRequest, res, next) => {
    ...
    const wishlist = await canAccessWishlist(req.user.userId, wish.wishlistId);
...
export const searchWishlist = asyncHandler(
  async (req: AuthenticatedRequest, res, next) => {
    ...
    const results = await searchWishlistByTitle(searchTerm, userId);
