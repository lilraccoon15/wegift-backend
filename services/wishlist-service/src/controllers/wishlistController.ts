...
export const searchWishlist = asyncHandler(
    async (req: AuthenticatedRequest, res, next) => {
        ...
        const results = await searchWishlistByTitle(
            searchTerm,
            userId,
            userRole
        );
...
export const leaveWishlist = asyncHandler(
    async (req: AuthenticatedRequest, res, next) => {
        ...
        if (!collaborator) {
            throw new AppError("Vous ne participez pas à cette wishlist", 404);
        }

        await collaborator.destroy();
        sendSuccess(res, "Vous avez quitté la wishlist", {});
    }
);
