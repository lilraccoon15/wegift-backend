...
export const deleteWishlistById = async (id: string, userId: string) => {
    ...
    if (wishlist.userId !== userId) {
        throw new AuthError(
            "Vous n’êtes pas autorisé à supprimer cette wishlist"
        );
    }
...
export const unsubscribeFromWishlistService = async (
    ...
    if (subscriberId === wishlist.userId) {
        throw new ValidationError(
            "Le propriétaire ne peut pas être désabonné."
        );
    }
...
export const reserveWishService = async (
    ...
    if (!wishlist) throw new NotFoundError("Liste non trouvée");
    ...
    try {
        ...
        console.error(
            `Erreur lors de l'envoi de la notification pour la réservation du souhait ${wishId} :`,
            error
        );
    }
